...
void *top_three_thread(void *arg){
        char* argv = (char *)arg;
        FILE * fp;
        char * line = NULL;
        size_t len = 0;
        ...
        FileContents *vectors = (FileContents *)malloc(sizeof(*vectors));
        // //Array of structs for the comparisons done
        WordComparison comps[4];
        ...
        while ((read = getline(&line, &len, fp)) != -1) {
            ...
            //Use strtok_r because it is thread safe
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i <= argc -1; i++){
        ...
        pthread_create(&thread[i], NULL, top_three_thread,  argv[i]);       
    }
    for(i = 0; i < argc  ; i++){
